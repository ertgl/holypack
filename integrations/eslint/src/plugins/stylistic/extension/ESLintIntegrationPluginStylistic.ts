...
import type { ESLintIntegrationPluginStylisticFacets } from "./ESLintIntegrationPluginStylisticFacets";
import { ESLintPluginStylisticConfiguratorFacet } from "./facets/ESLintPluginStylisticConfiguratorFacet";
import { INTEGRATION_UID_ESLINT_PLUGIN_STYLISTIC } from "./INTEGRATION_UID_ESLINT_PLUGIN_STYLISTIC";

export class ESLintIntegrationPluginStylistic extends AbstractExtension
{
  readonly $uid = INTEGRATION_UID_ESLINT_PLUGIN_STYLISTIC;

  readonly facets: ESLintIntegrationPluginStylisticFacets;

  readonly options: ESLintIntegrationPluginStylisticOptions;
  ...
  )
  {
    ...
    this.facets = {
      configurator: new ESLintPluginStylisticConfiguratorFacet(),
    };
  ...
  ): void
  {
    if (hook.name === ESLINT_INTEGRATION_HOOK_UID_GENERATE_LINTER_CONFIG_ARRAY_ASYNC)
    {
      ...
      (hook as GenerateLinterConfigArrayHookAsync).tapPromise(
        ...
        this.configureESLintPluginStylistic.bind(
          this,
          context,
        ),
      );
  ...
  ): void
  {
    if (hook.name === ESLINT_INTEGRATION_HOOK_UID_GENERATE_LINTER_CONFIG_ARRAY_SYNC)
    {
      ...
      (hook as GenerateLinterConfigArrayHookSync).tap(
        ...
        this.configureESLintPluginStylisticSync.bind(
          this,
          context,
        ),
      );
  ...
  async configureESLintPluginStylistic(
    context: ContextAsync,
