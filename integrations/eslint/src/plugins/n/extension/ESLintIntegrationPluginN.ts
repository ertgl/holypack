...
import type { ESLintIntegrationPluginNFacets } from "./ESLintIntegrationPluginNFacets";
import { ESLintPluginNConfiguratorFacet } from "./facets/ESLintPluginNConfiguratorFacet";
import { INTEGRATION_UID_ESLINT_PLUGIN_N } from "./INTEGRATION_UID_ESLINT_PLUGIN_N";

export class ESLintIntegrationPluginN extends AbstractExtension
{
  readonly $uid = INTEGRATION_UID_ESLINT_PLUGIN_N;

  readonly facets: ESLintIntegrationPluginNFacets;

  readonly options: ESLintIntegrationPluginNOptions;
  ...
  )
  {
    ...
    this.facets = {
      configurator: new ESLintPluginNConfiguratorFacet(),
    };
  ...
  ): void
  {
    if (hook.name === ESLINT_INTEGRATION_HOOK_UID_GENERATE_LINTER_CONFIG_ARRAY_ASYNC)
    {
      ...
      (hook as GenerateLinterConfigArrayHookAsync).tapPromise(
        ...
        this.configureESLintPluginN.bind(
          this,
          context,
        ),
      );
  ...
  ): void
  {
    if (hook.name === ESLINT_INTEGRATION_HOOK_UID_GENERATE_LINTER_CONFIG_ARRAY_SYNC)
    {
      ...
      (hook as GenerateLinterConfigArrayHookSync).tap(
        ...
        this.configureESLintPluginNSync.bind(
          this,
          context,
        ),
      );
