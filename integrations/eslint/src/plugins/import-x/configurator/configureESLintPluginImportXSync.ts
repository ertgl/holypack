...
export function configureESLintPluginImportXSync(
...
): void
{
  ...
  const pluginImportX = suppressErrorSync(
    requireDefaultExport<typeof ESLintPluginImportX>,
    require,
  ...
  if (!settingsConfigExists)
  {
    ...
    const createTypeScriptImportResolver = suppressErrorSync(
      () =>
      {
        const {
