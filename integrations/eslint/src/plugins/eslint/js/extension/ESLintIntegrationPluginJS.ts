...
import type { ESLintIntegrationPluginJSFacets } from "./ESLintIntegrationPluginJSFacets";
import { ESLintPluginJSConfiguratorFacet } from "./facets/ESLintPluginJSConfiguratorFacet";
import { INTEGRATION_UID_ESLINT_PLUGIN_JS } from "./INTEGRATION_UID_ESLINT_PLUGIN_JS";

export class ESLintIntegrationPluginJS extends AbstractExtension
{
  readonly $uid = INTEGRATION_UID_ESLINT_PLUGIN_JS;

  readonly facets: ESLintIntegrationPluginJSFacets;

  readonly options: ESLintIntegrationPluginJSOptions;
  ...
  )
  {
    ...
    this.facets = {
      configurator: new ESLintPluginJSConfiguratorFacet(),
    };
  ...
  ): void
  {
    if (hook.name === ESLINT_INTEGRATION_HOOK_UID_GENERATE_LINTER_CONFIG_ARRAY_ASYNC)
    {
      ...
      (hook as GenerateLinterConfigArrayHookAsync).tapPromise(
        ...
        this.configureESLintPluginJS.bind(
          this,
          context,
        ),
      );
  ...
  ): void
  {
    if (hook.name === ESLINT_INTEGRATION_HOOK_UID_GENERATE_LINTER_CONFIG_ARRAY_SYNC)
    {
      ...
      (hook as GenerateLinterConfigArrayHookSync).tap(
        ...
        this.configureESLintPluginJSSync.bind(
          this,
          context,
        ),
      );
