...
import type { ESLintIntegrationPluginGlobalIgnoresFacets } from "./ESLintIntegrationPluginGlobalIgnoresFacets";
import { ESLintPluginGlobalIgnoresConfiguratorFacet } from "./facets/ESLintPluginGlobalIgnoresConfiguratorFacet";
import { INTEGRATION_UID_ESLINT_PLUGIN_GLOBAL_IGNORES } from "./INTEGRATION_UID_ESLINT_PLUGIN_GLOBAL_IGNORES";

export class ESLintIntegrationPluginGlobalIgnores extends AbstractExtension
{
  readonly $uid = INTEGRATION_UID_ESLINT_PLUGIN_GLOBAL_IGNORES;

  readonly facets: ESLintIntegrationPluginGlobalIgnoresFacets;

  readonly options: ESLintIntegrationPluginGlobalIgnoresOptions;
  ...
  )
  {
    ...
    this.facets = {
      configurator: new ESLintPluginGlobalIgnoresConfiguratorFacet(),
    };
  ...
  ): void
  {
    if (hook.name === ESLINT_INTEGRATION_HOOK_UID_GENERATE_LINTER_CONFIG_ARRAY_ASYNC)
    {
      ...
      (hook as GenerateLinterConfigArrayHookAsync).tapPromise(
        ...
        this.configureESLintPluginGlobalIgnores.bind(
          this,
          context,
        ),
      );
  ...
  ): void
  {
    if (hook.name === ESLINT_INTEGRATION_HOOK_UID_GENERATE_LINTER_CONFIG_ARRAY_SYNC)
    {
      ...
      (hook as GenerateLinterConfigArrayHookSync).tap(
        ...
        this.configureESLintPluginGlobalIgnoresSync.bind(
          this,
          context,
        ),
      );
  ...
  async configureESLintPluginGlobalIgnores(
    context: ContextAsync,
