...
import type { ESLintIntegrationPluginYMLFacets } from "./ESLintIntegrationPluginYMLFacets";
import { ESLintPluginYMLConfiguratorFacet } from "./facets/ESLintPluginYMLConfiguratorFacet";
import { INTEGRATION_UID_ESLINT_PLUGIN_YML } from "./INTEGRATION_UID_ESLINT_PLUGIN_YML";

export class ESLintIntegrationPluginYML extends AbstractExtension
{
  readonly $uid = INTEGRATION_UID_ESLINT_PLUGIN_YML;

  readonly facets: ESLintIntegrationPluginYMLFacets;

  readonly options: ESLintIntegrationPluginYMLOptions;
  ...
  )
  {
    ...
    this.facets = {
      configurator: new ESLintPluginYMLConfiguratorFacet(),
    };
  ...
  ): void
  {
    if (hook.name === ESLINT_INTEGRATION_HOOK_UID_GENERATE_LINTER_CONFIG_ARRAY_ASYNC)
    {
      ...
      (hook as GenerateLinterConfigArrayHookAsync).tapPromise(
        ...
        this.configureESLintPluginYML.bind(
          this,
          context,
        ),
      );
  ...
  ): void
  {
    if (hook.name === ESLINT_INTEGRATION_HOOK_UID_GENERATE_LINTER_CONFIG_ARRAY_SYNC)
    {
      ...
      (hook as GenerateLinterConfigArrayHookSync).tap(
        ...
        this.configureESLintPluginYMLSync.bind(
          this,
          context,
        ),
      );
  ...
  async configureESLintPluginYML(
    context: ContextAsync,
